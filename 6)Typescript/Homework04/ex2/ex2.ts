...
class DictionaryElement<Key, Value, Description> {
    private _key: Key;
...
class Dictionary<Key, Value, Description> {
    private data: DictionaryElement<Key, Value, Description>[] = [];

    public add(key: Key, value: Value, description: Description) {
        let entry = new DictionaryElement<Key, Value, Description>();
        entry.key = key;
        entry.value = value;
    ...
    public getValueByKey(key: Key) : Value | null {
    ...
    public getDescriptionByValue(value: Value) : Description | null {
        for(let i = 0; i < this.data.length; i ++) {
    ...
    public getDescriptionByKey(key: Key) : Description | null {
        for(let i = 0; i < this.data.length; i ++) {
            if(this.data[i].key == key) {
                return this.data[i].description;
            }
    ...
    public getValueAndDescriptionByKey(key: Key) : String | null {
        for(let i = 0; i < this.data.length; i ++) {
            if(this.data[i].key == key) {
...
class DetailedDescription<Theme, Description> {
    constructor(public _theme: Theme, public _description: Description) {
        this._theme = _theme;
...
let otherDictionary = new Dictionary<String, String, DetailedDescription<String, String>> ();
